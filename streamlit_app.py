...
st.set_page_config(page_title="Point‑Process Entropy (Betting Correlation)", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🎲 Point‑Process Entropy (Betting Correlation)</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Binary directional predictability | Macro‑stratified betting correlation | Run entropy</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Point Process")
...
st.sidebar.markdown(f"**Macro bins:** {config.MACRO_BINS} | **All macros used**")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'point_process_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Betting correlation: {etf['pp_score_norm']:.3f}</p>
...
with tab1:
    st.header("🎲 Top ETFs by Betting Correlation (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Point‑process entropy** models the binary sequence of up/down days.
        - **Betting correlation** measures how well past direction predicts future direction (phi coefficient).
        - For each macro variable, we compute betting correlation separately for low/medium/high macro regimes.
        - The per‑ETF score is the **weighted average of regime‑contrast** across all macro variables (weights from ridge regression).
        - High score → direction is highly predictable given macro regime.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the betting correlation scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Point‑Process Entropy | Binary directional predictability with macro stratification")
